...
export default function AttackModal({
  ...
  const dialogTitleId = `attack-modal-title-${targetMembershipId}`
  const dialogDescriptionId = `attack-modal-description-${targetMembershipId}`
  const searchInputId = `attack-modal-search-${targetMembershipId}`
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-lg bg-white dark:bg-brand-card-dark rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden max-h-[90vh] flex flex-col overscroll-contain"
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id={dialogTitleId} className="font-heading font-black text-lg text-gray-900 dark:text-white leading-tight">
                هجوم على {targetAlias}
              </h2>
              <p id={dialogDescriptionId} className="text-xs text-gray-400 font-medium">خمّن هويته الحقيقية</p>
            </div>
          </div>
          <button
            onClick={onClose}
            type="button"
            aria-label="إغلاق نافذة الهجوم"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 inventory-scroll">
          ...
          {/* ── Identity search ── */}
          <div>
            <label htmlFor={searchInputId} className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-2">
              اختر الاسم الحقيقي
            </label>
          ...
          {/* Execute error */}
          {executeError && (
            <p role="alert" className="text-sm text-brand-danger font-bold text-center">{executeError}</p>
          )}
        </div>

        {/* ── Confirm button — pinned to bottom ── */}
        <div className="p-6 pt-3 border-t border-gray-100 dark:border-gray-800 flex-shrink-0 safe-area-pb">
          <button

...
function RuleItem({ item }) {
  return (
    <div className="flex items-start gap-3 group">
      <div className={`w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:bg-white/10 smooth-transition ${item.color || ''}`}>
        <iconify-icon
      ...
      <div className="flex-1 min-w-0">
        {item.label && (
          <span className="font-heading font-black text-sm text-white block mb-0.5">{item.label}</span>
        )}
        <p className="text-sm text-gray-300 leading-relaxed font-medium">{item.text}</p>
      </div>
    </div>
  )
}
...
function RuleSection({ section, index }) {
  return (
    <section id={section.id} className="relative">
      {/* Section card */}
      <div className="relative bg-white/[0.03] border border-white/[0.06] rounded-3xl p-6 md:p-10 overflow-hidden group hover:bg-white/[0.05] smooth-transition">
        {/* Glow */}
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm text-gray-400 font-bold mb-8 mr-16">{section.subtitle}</p>
...
export default function RulesPage() {
  return (
    <div
      ...
      style={{ background: 'linear-gradient(180deg, #0a0d14 0%, #111827 50%, #0a0d14 100%)' }}
    >
      ...
      {/* Floating shapes */}
      <div className="absolute top-40 left-10 w-72 h-72 bg-brand-teal/5 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-60 right-10 w-80 h-80 bg-brand-orange/5 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-purple-500/3 rounded-full blur-[150px] pointer-events-none"></div>

      <div className="relative z-10 max-w-4xl mx-auto px-4 py-10 md:py-16 space-y-10">

        {/* ── Hero ── */}
        <header className="text-center space-y-6 mb-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-black text-gray-400 uppercase tracking-[0.15em]">
            <iconify-icon icon="lucide:book-open" class="text-brand-teal text-sm"></iconify-icon>
            دليل اللاعب
          </div>
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-black text-white leading-[1.1] tracking-tight">
            قواعد
        ...
        <nav className="flex flex-wrap justify-center gap-2 pb-4">
          {SECTIONS.map((s) => (
            <a
              key={s.id}
              href={`#${s.id}`}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/5 border border-white/[0.08] text-xs font-bold text-gray-400 hover:text-white hover:bg-white/10 hover:border-white/20 smooth-transition"
            >
        ...
        <div className="text-center space-y-6 pt-8 pb-4">
          <div className="w-16 h-px bg-gradient-to-l from-transparent via-gray-600 to-transparent mx-auto"></div>
          <p className="text-gray-400 font-bold text-lg">جاهز للمعركة؟</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              to="/store"
              className="px-8 py-4 bg-white/5 border border-white/10 text-gray-300 font-heading font-bold text-lg rounded-2xl hover:bg-white/10 hover:text-white flex items-center gap-3 smooth-transition hover:-translate-y-1"
            >

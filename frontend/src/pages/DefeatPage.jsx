import { Link, useLocation } from 'react-router-dom'

export default function DefeatPage() {
  const { state } = useLocation()
  const penalty = state?.penalty_amount ?? 0
  const attackerBalanceAfter = state?.attacker_balance_after ?? null

  return (
    <div className="flex-1 w-full max-w-4xl mx-auto px-4 py-12 md:py-20 flex flex-col items-center">
      <div className="w-full flex flex-col items-center text-center space-y-12">

        {/* Hero Section */}
        <div className="relative">
          <div className="absolute inset-0 bg-brand-danger/15 dark:bg-brand-danger/10 blur-3xl rounded-full"></div>
          <div className="relative floating">
            <div className="bg-gradient-to-br from-brand-danger to-red-600 text-white w-24 h-24 md:w-32 md:h-32 rounded-full flex items-center justify-center shadow-xl shadow-brand-danger/20 border-4 border-white dark:border-brand-card-dark mx-auto z-10 relative">
              <iconify-icon icon="lucide:x-circle" class="text-4xl md:text-5xl drop-shadow-md"></iconify-icon>
            </div>
          </div>
          <h1 className="font-display text-5xl md:text-7xl font-black text-brand-danger mt-8 tracking-tight drop-shadow-sm uppercase">
            هزيمة!
          </h1>
          <p className="text-gray-500 dark:text-gray-400 font-medium text-lg md:text-xl mt-4 max-w-md mx-auto">
            للأسف خسرت هذه المعركة — تخمينك كان خاطئاً. حاول مرة أخرى!
          </p>
        </div>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">

          {/* Points Lost Card */}
          <div className="bg-white dark:bg-brand-card-dark border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-sm hover:shadow-md dark:hover:shadow-black/20 flex flex-col items-center gap-4 smooth-transition hover:-translate-y-1 group">
            <span className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-brand-danger transition-colors">النقاط المفقودة</span>
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-red-50 dark:bg-brand-danger/10 flex items-center justify-center text-brand-danger">
                <iconify-icon icon="lucide:zap" class="text-3xl"></iconify-icon>
              </div>
              <span className="font-display text-5xl md:text-6xl font-black text-gray-900 dark:text-white drop-shadow-sm">-{penalty}</span>
            </div>
            <div className="h-px w-full bg-gray-100 dark:bg-gray-800 my-2"></div>
            <div className="flex justify-between w-full text-sm font-bold">
              <span className="text-gray-500 dark:text-gray-400">رصيدك الحالي:</span>
              <span className="text-brand-danger font-black">
                {attackerBalanceAfter !== null ? attackerBalanceAfter.toLocaleString('ar-EG') : '—'}
              </span>
            </div>
          </div>

          {/* Wrong guess info */}
          <div className="bg-white dark:bg-brand-card-dark border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-sm hover:shadow-md dark:hover:shadow-black/20 flex flex-col items-center gap-4 smooth-transition hover:-translate-y-1 group">
            <span className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-brand-danger transition-colors">نتيجة التخمين</span>
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-red-50 dark:bg-brand-danger/10 flex items-center justify-center text-brand-danger">
                <iconify-icon icon="lucide:user-x" class="text-3xl"></iconify-icon>
              </div>
              <span className="font-display text-3xl md:text-4xl font-black text-gray-900 dark:text-white drop-shadow-sm">خطأ!</span>
            </div>
            <div className="h-px w-full bg-gray-100 dark:bg-gray-800 my-2"></div>
            <p className="text-sm text-gray-400 font-medium text-center">
              الهدف لا يزال محتفظاً بهويته الحقيقية
            </p>
          </div>
        </div>

        {/* CTA Buttons */}
        <div className="flex flex-col md:flex-row gap-4 w-full pt-6">
          <Link to="/dashboard" className="flex-1 btn-press bg-brand-teal hover:bg-brand-teal-hover text-white py-4 md:py-5 rounded-2xl font-heading font-black text-lg shadow-lg shadow-brand-teal/20 smooth-transition text-center flex items-center justify-center gap-3">
            العودة للرئيسية
            <iconify-icon icon="lucide:arrow-left" class="text-xl"></iconify-icon>
          </Link>
          <Link to="/leaderboard" className="flex-1 btn-press bg-white dark:bg-brand-card-dark border-2 border-gray-200 hover:border-brand-teal dark:border-gray-700 dark:hover:border-brand-teal text-gray-700 hover:text-brand-teal dark:text-gray-300 dark:hover:text-brand-teal py-4 md:py-5 rounded-2xl font-heading font-black text-lg shadow-sm hover:shadow-md smooth-transition text-center flex items-center justify-center gap-3">
            جرب لاعباً آخر
            <iconify-icon icon="lucide:swords" class="text-xl"></iconify-icon>
          </Link>
        </div>
      </div>
    </div>
  )
}
